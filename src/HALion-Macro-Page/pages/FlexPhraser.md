...
|**Load From**|Load From specifies the subpath to the location of the subpresets inside the root folder. The root folder is set by Product (see above). You can specify this subfolder freely. However, if you want to see the factory phrases, Load From must be set to ``StepSEQ Phrases`` and Product must contain ``HALion``.<p>Any user phrases for your library must be added to the corresponding location inside the VST Sound. For example, if Load From is set to ``StepSEQ Phrases`` and Product is set just to ``MyProduct``, the subpresets inside the VST Sound must be located at ./*``MyProduct``*/Sub Presets/*``StepSEQ Phrases``*.</p>|
